...
def fitness(individual, obs_res, obs_unres, obs_res_err, obs_unres_err):
    ...
    n_bands_resolved = len(obs_res[0])
    ...
    for i in np.arange(n_sources):
        ...
        prior_t *= prior_age(np.log10(age),5.5,1.0) # Prior on age
        prior_obs *= prior_Av(np.log10(individual[i][2]),float(sys.argv[3]),float(sys.argv[4])) # Prior on Av
    ...
    # Calculate chi squared for resolved band
    ...
    for source in np.arange(n_sources):
        ...
        *obs_res_err[source][zeros_index_res[source]]*obs_res_err[source][zeros_index_res[source]] 
        ...
        log_mod_flux = np.log10(model_fluxes_all[source][0:n_bands_resolved][zeros_index_res[source]]) 
    ...
    mod_flux_upper = 0.0
    for source in np.arange(n_sources): 
        #print n_bands_resolved
        #print "model = ",model_fluxes_all[source][n_bands_resolved:]
        mod_flux_upper += model_fluxes_all[source][n_bands_resolved:][zeros_index_unres][ind_upper_unres]
    ...
    *obs_unres_err[zeros_index_unres]*obs_unres_err[zeros_index_unres] 
    ...
    for source in np.arange(n_sources): 
        mod_flux += model_fluxes_all[source][n_bands_resolved:][zeros_index_unres]
...
p = population(40,len(obs_res),0.8,1.3,float(sys.argv[2]),0.02)
...
for i in xrange(100000): # Number of generations
    if (i%10 == 0): print i
