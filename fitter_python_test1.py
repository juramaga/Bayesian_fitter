...
def likelihood(obs_fluxes_resolved, obs_errors_resolved, obs_fluxes_unresolved, 
    ...
    zeros_index = np.array(np.where(obs_fluxes_unresolved == 0.0))
    ...
    for source in np.arange(n_sources): model_fluxes[source] = np.delete(model_fluxes[source],n_bands_resolved+zeros_index)
    
    for source in np.arange(n_sources):
        zeros_index = np.array(np.where(obs_fluxes_resolved[source] == 0.0))
    ...
    for source in np.arange(n_sources):
        ...
        log_obs_flux = np.log10(obs_fluxes_resolved[source]) \
        -0.5*(1.0/np.log(10.))*(1.0/(obs_fluxes_resolved[source]**2.0)) \
        *obs_errors_resolved[source]*obs_errors_resolved[source]
        ...
        log_obs_var2 = (((1.0/np.log(10.))*(1.0/obs_fluxes_resolved[source]))**2.0) \
        *obs_errors_resolved[source]*obs_errors_resolved[source] 
    
        # Model flux in log space
        log_mod_flux = np.log10(model_fluxes[source][0:n_data_res]) 
    ...
    # Unbiased flux in log space (since we are fitting in logarithmic space, see Robitaille et al. 2007)
    log_obs_flux = np.log10(obs_fluxes_unresolved)-0.5*(1.0/np.log(10.)) \
    *(1.0/(obs_fluxes_unresolved**2.0))*obs_errors_unresolved*obs_errors_unresolved
        
    # Unbiased variances in log space (since we are fitting in logarithmic space, see Robitaille et al. 2007)
    log_obs_var2 = (((1.0/np.log(10.))*(1.0/obs_fluxes_unresolved))**2.0) \
...
def mh(Ns, x0, filter_names, obs_fluxes_resolved, obs_errors_resolved, obs_fluxes_unresolved, obs_errors_unresolved):
    ...
    for i in np.arange(n_sources):
        ...
        prior_obs_0 *= prior_Av(np.log10(xsample[2,i,0]),1.3,1.2)
    ...
    while cnt+1 < Ns:
        ...
        for i in np.arange(n_sources):
            ...
            prior_obs *= prior_Av(np.log10(xstar[2]),1.3,1.2)
...
hdulist = fits.open('r08-objects.fits')
tbdata = hdulist[1].data
mags_spitzer = np.array([tbdata[127]['__3_6_G'],tbdata[127]['__4_5_'],tbdata[127]['__5_8_G'],tbdata[127]['__8_0_'],tbdata[127]['__24_']])
zero_points = np.array([280.9, 179.7,115.0,64.13,7.14])
flux_spitzer =  zero_points*10.0**(-mags_spitzer/2.5)

hdulist = fits.open('ukidss-sources.fits')
mags_ukidss = np.array([hdulist[1].data[443:446]['mag_j'],hdulist[1].data[443:446]['mag_h'],hdulist[1].data[443:446]['mag_k']])
err_ukidss = np.array([hdulist[1].data[443:446]['emag_j'],hdulist[1].data[443:446]['emag_h'],hdulist[1].data[443:446]['emag_k']])
...
obs_res = 1000.0*np.transpose(flux_ukidss)
...
# Initial guesses
#init = [[3000816, 0, 13.05,0.6,0.6],[3004924, 2, 42.00,0.6,0.6],[3007450, 0, 23.30,0.6,0.6]]
#init = [[12625, 1, 10.46, d_src, d_src],[16132, 1, 28.55, d_src, d_src],[12610, 1, 16.63 ,d_src ,d_src]]
#init = [[7296, 3, 11.51 ,d_src ,d_src],[5156, 0, 8.45, d_src, d_src],[2029, 1, 12.21, d_src, d_src]]
init = [[7296, 3, 21.51 ,0.6 ,0.6],[5156, 0, 18.45, 0.6, 0.6],[2029, 1, 22.21, 0.6, 0.6]]
N = 100000
